  ...
  config = {
    ...
    users = {
      ...
      groups = mkMerge (
        ...
        ++ (
          let
            members = flatten (
              mapAttrsToList (
                n: u: map (g: { "${g}".members = [ n ]; }) (u.extraGroups ++ singleton u.group)
              ) cfg.users
            );
          in
          members
      ...
      groupFile = pkgs.writeText "group" (
        concatStringsSep "\n" (
          mapAttrsToList (n: v: with v; "${n}:x:${toString gid}:${concatStringsSep "," members}") cfg.groups
        )
      );
      generateShadow = pkgs.writeShellScript "generate-shadow" ''
        ...
        ${concatStringsSep "\n" (
          mapAttrsToList (
            ...
            with v;
            "${n}:${
              if hashedPassword != null then
                "$(echo '${hashedPassword}')"
              else if hashedPasswordFile != null then
                "$(<${hashedPasswordFile})"
              else
                "!"
            }:1::::::"
