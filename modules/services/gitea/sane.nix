    ...
    (
      { config, ... }:
      {
          ...
          database = mkOption {
            type = taggedSubmodules {
              types.mysql = types.submodule {
                options = {
                  type = mkOption { type = types.enum [ "mysql" ]; };

                  name = mkOption { type = with types; str; };

                  user = mkOption { type = with types; str; };
              ...
              types.postgres = types.submodule {
                options = {
                  type = mkOption { type = types.enum [ "postgres" ]; };
              ...
              types.sqlite3 = types.submodule {
                options = {
                  type = mkOption { type = types.enum [ "sqlite3" ]; };
        ...
        config.output = {
          assertions =
            let
              remoteDatabase = elem config.database.type [
                "postgresql"
                "mysql"
              ];
              requiredSecrets = [
          ...
          services.gitea = {
            settings = {
              ...
              database = mkMerge [
                ...
                (mkIf (config.database.type == "sqlite") { PATH = config.database.path; })
                (mkIf (config.database.type == "postgres") { SSL_MODE = "disable"; })
              ];
              ...
              lfs = mkIf config.lfs.enable { PATH = config.lfs.contentDirectory; };
            };
          };
        };
      }
